...
#' @param na.rm logical. Should leading `NA`s be removed?
#' @param fromLast logical. Causes observations to be carried backward rather
#' than forward.  Default is `FALSE`.  With a value of `TRUE` this
#' corresponds to NOCB (next observation carried backward).  It is not
#' supported if `x` or `xout` is specified.
#' @param rev Use `fromLast` instead.  This argument will be eliminated in
...
#' `maxgap` `NA`s while filling other `NA`s with the last
#' occurrence of a non-`NA`.)
#' @param rule See [approx()].
...
#' non-`NA`s then the `NA` is omitted (if `na.rm = TRUE`) or it
#' is not replaced (if `na.rm = FALSE`).
#' 
...
#' `NA` then with `na.rm = TRUE`, the default, the above implies that
#' the resulting object will have zero rows. Use `na.rm = FALSE` to
#' preserve the `NA` values instead.
#' @seealso [zoo()]
#' @keywords ts
#' @examples
#' 
...
na.locf <- function(object, na.rm = TRUE, ...) UseMethod("na.locf")
